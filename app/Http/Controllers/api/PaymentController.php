...
class PaymentController extends Controller
{
    ...
    public function getAllPackages(request $request)
    {
        ...
        if($request->language_code=="en"){
            ...
            $packages = Package::select('id', 'package_name', 'package_id'
                ...
                , 'max_podcast_no', 'sequence', 'is_delete','allowed_routes','allowed_podcasts')->where('is_delete', 0)->orderBy('sequence', "ASC")->get();
    ...
    public function getMyOrders(request $request)
    {
        ...
        if($request->language_code=="en"){
            ...
            //dd($list->package_details_on_purchase_time['package_id']);
            foreach($list as $val){
                $srch_package=DB::table('packages')->where('id',$val->package_details_on_purchase_time['package_id'])->first();
                //dd($srch_package);
                if($srch_package->is_delete!="1"){
                    $val['updated_active_image']=$srch_package->package_image;
                    array_push($a, $val);
                }
            }
           // dd($a);
        ...
        elseif ($request->language_code=="fr") {
            ...
            ->where('user_id', $userId)->orderBy('id', "DESC")->get();
              foreach($list as $val){
                $srch_package=DB::table('french_roadtrip_packages')->where('id',$val->package_details_on_purchase_time['package_id'])->first();
                //dd($srch_package);
                if($srch_package->is_delete!="1"){
                   $val['updated_active_image']=$srch_package->package_image;
                    array_push($a, $val);
        ...
        elseif ($request->language_code=="de") {
            ...
            ->where('user_id', $userId)->orderBy('id', "DESC")->get();
               foreach($list as $val){
                $srch_package=DB::table('german_roadtrip_packages')->where('id',$val->package_details_on_purchase_time['package_id'])->first();
                //dd($srch_package);
                if($srch_package->is_delete!="1"){
                    $val['updated_active_image']=$srch_package->package_image;
                    array_push($a, $val);
        ...
        if (count($list) > 0) {
            $response = array("flag" => true, "status_code" => 201, "message" => "data found", "body" => $a);
        } else {
    ...
    public function insert_data_to_user_road_trip(Request $request){
       ...
       $validator = Validator::make($request->all(), [
        ...
        foreach($srch as $value)
        {
         ...
         //inserted roadtrip  = srch_roadtrip_count
         //inserted podcast =srch_podcast_count
         //available total roadtrip = c_route
         //availabale total podcast = c_podcast
       // dd( $srch_roadtrip_count,$srch_podcast_count,$c_route,$c_podcast);
         ...
         elseif($request->roadtrip_status=="0" && $request->podcast_status=="1" ){
            ...
            if($chk_already_podcast_id_exists<1){
               ...
               if($srch_podcast_count>=$c_podcast){   //+1 for one free
                 return response()->json(['message'=>"Subscribe package to download to the podcasts.","status"=>"error"], 401); 
               }else{
...
//insert podcast and routes ids for count of used, that user using under a package.
public function user_used_data_count(Request $request)
{
    $validator = Validator::make($request->all(), [
    ...
    $usage = UserPackageUsage::firstOrNew([
        'user_id'    => (int) $request->user_id,
        'package_id' => (int) $request->package_id,
    ]);

    if (!$usage->exists) {
        $usage->used_podcast_count = 0;
        $usage->used_daily_routes_count = 0;
        $usage->used_podcast_ids = [];
        $usage->used_daily_routes_ids = [];
    }

    //  Handle allowed_routes (all / numeric)
    if ($packageDetails->allowed_routes === "all") {
        $routeCount = Route::where('route_status', 1)->count();
    } else {
        $routeCount = (int)$packageDetails->allowed_routes;
    }

    //  Handle allowed_podcasts (all / numeric)
    if ($packageDetails->allowed_podcasts === "all") {
        $podcastCount = Podcast::where('pod_active_status', '1')->count();
    } else {
        $podcastCount = (int)$packageDetails->allowed_podcasts;
    }

    // Podcast logic
    if ($request->podcast_id) {
        $ids = $usage->used_podcast_ids ?? [];
        if (!in_array((int)$request->podcast_id, $ids)) {
            if ((int)$usage->used_podcast_count >= $podcastCount) {
                return response()->json([
                    'success' => false,
                    'message' => 'Maximum number of podcasts used.',
                    'allowed_routes' => $routeCount,
                    'allowed_podcasts' => $podcastCount,
                ]);
            }
            $ids[] = (int)$request->podcast_id;
            $usage->used_podcast_ids = $ids;
            $usage->used_podcast_count++;
        }
    }

    // Route logic
    if ($request->route_id) {
        $ids = $usage->used_daily_routes_ids ?? [];
        if (!in_array((int)$request->route_id, $ids)) {
            if ((int)$usage->used_daily_routes_count >= $routeCount) {
                return response()->json([
                    'success' => false,
                    'message' => 'Maximum number of routes used.',
                    'allowed_routes' => $routeCount,
                    'allowed_podcasts' => $podcastCount,
                ]);
            }
            $ids[] = (int)$request->route_id;
            $usage->used_daily_routes_ids = $ids;
            $usage->used_daily_routes_count++;
        }
    }
...
public function user_used_data_count_get(Request $request){
    $validator = Validator::make($request->all(), [
            'user_id' => 'required|integer',
            'package_id' => 'required|integer',
        ]);
      ...
      // route
        ...
        if($usage){
            return response()->json([
                'success' => true,
                'data' => $usage,
                'allowed_routes'=>(int)$routeCount,
                'allowed_podcasts'=>(int)$podcastCount,
                // 'all_route'=>$all_route,
            ...
            $obj = [
                "user_id" => (int)$request->user_id,
                "package_id" => (int)$request->package_id,
                "used_podcast_count" => 0,
                "used_daily_routes_count" => 0,
                "used_podcast_ids" => [],
