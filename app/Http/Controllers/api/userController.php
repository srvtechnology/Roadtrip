...
class userController extends Controller
{
    ...
    public function login(request $request)
    {
        ...
        if ($validator->fails()) {  
        return response()->json(['flag' => "false", 'status_code' => '1001', 'message' =>"Please enter valid email address"], 401); 
            ...
            $user = User::select($userSelectedArrayLogIn)
               ...
               //if the email is wrong.
                ...
                elseif($request->language_code=="fr"){
                    ...
                    $response = array('flag' => "false", 'status_code' => '1001', 'message' => htmlentities(json_decode($json), ENT_COMPAT, 'UTF-8') );
        ...
        if ($signUpType == $signUpTypeArray[2]) {
            ...
            if (strlen($password) < 8) {
                ...
                elseif($request->language_code=="fr"){
                    ...
                    $msg = htmlentities(json_decode($json), ENT_COMPAT, 'UTF-8') ;
                    array_push($errorList, $msg );
                }

                elseif($request->language_code=="de"){
                   $json='" Das Passwort muss mindestens 8 Zeichen enthalten "';
    ...
    public function sendFogotPasswordMail($email,$lan_code)
    {
        //dd($email,$lan_code);
         header('Content-Type: text/html; charset=utf-8');
        ...
        if($lan_code=="en"){
        $mailData = array(
            'organisationName' => 'Road Trip USA',
            'organisationEmail' => 'roadtrip.usa.southwest@gmail.com',
            'title' => 'Reset Password on Roadtrip USA',
    ...
    }//language code if english



    if($lan_code=="fr"){

        $json=' Réinitialiser le mot de passe ';
        $subject= htmlentities(json_decode($json), ENT_COMPAT, 'UTF-8');
        ...
        $mailData = array(
            ...
            'userEmail' => $email,
            'url' => "url",
            'emailHeaderSubject' => $json,
        );
        Mail::send('emails.forget_pass_french', $mailData, function ($message)
             use ($mailData) {
                $message->from($mailData['organisationEmail'], $mailData['organisationName']);
                $message->to($mailData['userEmail'], $mailData['organisationName'])->subject(config('global.SITE_ADDRESS_NAME') . $mailData['emailHeaderSubject']);
            });
    }//language code if french



    if($lan_code=="de"){
         $json=' Passwort zurücksetzen ';
        $subject= htmlentities(json_decode($json), ENT_COMPAT, 'UTF-8');
        ...
        Mail::send('emails.forget_pass_german', $mailData, function ($message)
             use ($mailData) {
                $message->from($mailData['organisationEmail'], $mailData['organisationName']);
                $message->to($mailData['userEmail'], $mailData['organisationName'])->subject(config('global.SITE_ADDRESS_NAME') . $mailData['emailHeaderSubject']);
            });
    }//language code if german
    ...
    public function forgotPassword(request $request)
    {
        ...
        $email = $request->input('email');
        $lan_code=$request->language_code;
        ...
        if (isset($email)) {
            ...
            if ($user) {
                ...
                if ($user->signup_type == 'NL') {
                    ...
                    $response = array('flag' => "true", 'status_code' => '201'
                        , 'message' => "A reset password link is send to your email id.",
                    );
                    ...
                    //self::sendMail($email);
                    self::sendFogotPasswordMail($email,$lan_code);
