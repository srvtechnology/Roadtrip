...
class userController extends Controller
{
    ...
    public function forgot_password_mail($serect_key, $email, $secutity_token,$lc)
    {
        // dd($lc);
        ...
        if ($chk_secret_key==$serect_key && $decode_email==$secutity_token ) {
            ...
            if ($chk_link == '1') {
            	# code...
            	return view('update_password',['email' => $decode,'lc'=>$lc]);
            }else{
            	return view('token_expired',['lc'=>$lc]);
    ...
    public function update_password(Request $request)
    {
        ...
        if (empty($request->password) || empty($request->confirm_password)) {
           ...
           elseif($request->lc=="fr"){
             return Response()->json(['flag' => 'false', "message" => "Remplir tous les champs"], Response::HTTP_OK);

           }
           elseif($request->lc=="de"){
             return Response()->json(['flag' => 'false', "message" => "Bitte alles ausfüllen"], Response::HTTP_OK);
            ...
            # code...
                ...
                elseif($request->lc=="fr"){
                     return Response()->json(['flag' => 'false', "message" => "Mot de passe doit être d’au moins 8
caractères
"], Response::HTTP_OK);

                }
                 elseif($request->lc=="de"){
                     return Response()->json(['flag' => 'false', "message" => "Passwort muss mindestens 8 Zeichen
haben"], Response::HTTP_OK);
            ...
            if ($request->password != $request->confirm_password) {
                ...
                elseif($request->lc=="fr"){
                     return Response()->json(['flag' => 'false', "message" => "Répéter le mot de passe"], Response::HTTP_OK);

                }
                elseif($request->lc=="de"){
                     return Response()->json(['flag' => 'false', "message" => "Wiederhole das neue Passwort"], Response::HTTP_OK);
            ...
            # code...
                    $data=User::where('email',$request->user_email)->where('user_active_status','1')->get();
                     ...
                     if ($user->save()) {
                         # code...
                        return Response()->json(['flag' => 'true', "message" => "Your password successfully updated","lc"=>$request->lc], Response::HTTP_OK);
                     }else{
    ...
    public function privacy_policy_de()
    {

        return view('privacy_policy_de');

    }

    public function terms_condition_de()
    {

        return view('terms_condition_de');

    }
     public function about_de()
    {

        return view('about_de');
    ...
    public function privacy_policy_fr()
    {

        return view('privacy_policy_fr');

    }

    public function terms_condition_fr()
    {

        return view('terms_condition_fr');

    }
     public function about_fr()
    {

        return view('about_fr');
